...
class SmsDemo
{
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = "LTAIlniVdZQtZsAy"; // AccessKeyId

        $accessKeySecret = "RouMi3ywOLsHPapKtfQz2Hu5OcgRfz"; // AccessKeySecret
    ...
    public static function sendSms($phone, $code, $type) {
        ...
        $request->setSignName("金尊娱乐");

        // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
        if($type == 0){
            $request->setTemplateCode("SMS_164276021"); //绑定手机号
        }elseif ($type == 1){
            $request->setTemplateCode("SMS_164276018"); //找回密码
        }elseif ($type == 2){
            $request->setTemplateCode("SMS_164266395"); //注册成为会员
        }elseif ($type == 3){
            $request->setTemplateCode("SMS_164266391"); //绑定支付宝银行卡收款账户
        }elseif ($type == 4){
            $request->setTemplateCode("SMS_164266390"); //绑定支付宝银行卡收款账户
        }else{
            $request->setTemplateCode("SMS_164266395");//默认注册成为会员
        }
